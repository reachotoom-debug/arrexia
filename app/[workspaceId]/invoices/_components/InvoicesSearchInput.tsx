"use client";

import { useRouter, useSearchParams } from "next/navigation";
import { useCallback, useEffect, useState } from "react";

interface InvoicesSearchInputProps {
  workspaceId: string;
  initialSearch?: string;
}

/**
 * Live search input that updates URL params as user types
 * Matches Clients/Payments page behavior for live filtering
 */
export function InvoicesSearchInput({
  workspaceId,
  initialSearch = "",
}: InvoicesSearchInputProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  // Initialize from URL params, fallback to prop
  const initialSearchTerm = searchParams.get("search") || initialSearch;
  const [searchTerm, setSearchTerm] = useState(initialSearchTerm);

  // Sync with URL params when they change externally (e.g., navigation)
  useEffect(() => {
    const urlSearch = searchParams.get("search") || "";
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setSearchTerm(prev => (prev === urlSearch ? prev : urlSearch));
  }, [searchParams]);

  const updateSearch = useCallback(
    (value: string) => {
      const params = new URLSearchParams(searchParams.toString());
      
      if (value.trim()) {
        params.set("search", value.trim());
      } else {
        params.delete("search");
      }
      
      // Reset to page 1 when searching
      params.set("page", "1");
      
      const query = params.toString();
      router.push(`/${workspaceId}/invoices${query ? `?${query}` : ""}`);
    },
    [router, workspaceId, searchParams]
  );

  // Debounce search updates
  useEffect(() => {
    const currentSearch = searchParams.get("search") || "";
    if (searchTerm === currentSearch) {
      return; // Already in sync with URL
    }

    const timer = setTimeout(() => {
      updateSearch(searchTerm);
    }, 300); // 300ms debounce

    return () => clearTimeout(timer);
  }, [searchTerm, searchParams, updateSearch]);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchTerm(e.target.value);
  };

  return (
    <input
      type="text"
      value={searchTerm}
      onChange={handleChange}
      onKeyDown={(e) => {
        if (e.key === "Enter") {
          e.preventDefault();
          updateSearch(searchTerm);
        }
      }}
      placeholder="Search invoice #, PO, notes..."
      className="h-10 w-full min-w-0 max-w-full rounded-lg border border-slate-200 px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-0 md:max-w-md lg:max-w-[28rem]"
    />
  );
}

