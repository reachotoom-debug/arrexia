-- ============================================================================
-- Finalize invoices_view to represent ACTIVE invoices only (AR integrity)
-- ============================================================================
-- 
-- This migration ensures invoices_view ALWAYS excludes archived invoices
-- to maintain financial integrity. invoices_view represents ACTIVE invoices only.
--
-- CRITICAL: The WHERE i.archived_at IS NULL clause MUST be present
-- in the invoice_calculations CTE to exclude archived invoices.
--
-- CRITICAL: Paid calculation must exclude archived payments (p.archived_at IS NULL).
-- ============================================================================
...
COMMENT ON VIEW public.invoices_view IS 
  'Canonical invoices view with stable contract. Represents ACTIVE invoices only (excludes archived). Required columns: id, workspace_id, client_id, client_name, invoice_number, issue_date, due_date, base_status (draft/sent/void), display_status (paid/partially_paid/overdue/sent/draft/void), total, paid, outstanding, currency (nullable), po_number (optional), notes (optional), is_overdue (boolean), overdue_days, risk_level (high/medium/low/null). ONLY includes non-archived invoices (WHERE i.archived_at IS NULL). Paid amount excludes archived payments (p.archived_at IS NULL). Draft invoices are NEVER treated as overdue or risk, even if due_date is in the past. Display status: void (highest priority), draft (protected from overdue), paid (outstanding <= 0), overdue (sent + outstanding > 0 + due_date < current_date), partially_paid (sent + outstanding > 0 + due_date >= current_date + paid > 0), sent (sent + outstanding > 0 + due_date >= current_date + paid = 0). Risk levels: high (overdue_days >= 60 OR outstanding >= 5000), medium (overdue_days 15-59), low (overdue_days 1-14), NULL (not overdue).';
