-- ============================================================================
-- Invoice import financial hardening (IMP-001 + IMP-002)
-- ============================================================================
-- IMP-001: Re-import paid-total guard under public.invoices FOR UPDATE on execute
-- IMP-002: Authoritative positive line-item validation (quantity, unit_price, totals)
-- Preserves: dry-run non-mutating preview, client resolution, no auto-create,
-- batch atomicity, entitlement wrapper unchanged.
-- ============================================================================
...
BEGIN
    ...
    IF v_rt = 'item' THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        v_errors := v_errors || jsonb_build_array(
          format('Invalid quantity for %s (must be a positive number)', v_inv)
        );
        CONTINUE;
      END;
      ...
      EXCEPTION WHEN OTHERS THEN
        v_errors := v_errors || jsonb_build_array(
          format('Invalid unit_price for %s (must be a positive number)', v_inv)
        );
        CONTINUE;
      END;
      ...
      IF v_quantity IS NULL OR v_quantity <= 0 THEN
        v_errors := v_errors || jsonb_build_array(
          format('Quantity must be greater than zero for %s', v_inv)
        );
        CONTINUE;
      END IF;
      ...
      IF v_unit_price IS NULL OR v_unit_price <= 0 THEN
        v_errors := v_errors || jsonb_build_array(
          format('Unit price must be greater than zero for %s', v_inv)
        );
        CONTINUE;
      END IF;
      ...
      IF v_line_amount <= 0 THEN
        v_errors := v_errors || jsonb_build_array(
          format('Line amount must be greater than zero for %s', v_inv)
        );
      END IF;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF v_new_total <= 0 THEN
      v_errors := v_errors || jsonb_build_array(
        format('Invoice %s must have a positive total (sum of line items)', v_inv)
      );
    END IF;
  END LOOP;
  ...
  IF p_dry_run THEN
    -- Paid-total guard (preview only; no row lock — best-effort for UX)
    FOR v_row IN
      SELECT value
      FROM jsonb_array_elements(p_rows) AS t(value)
      WHERE LOWER(COALESCE(value->>'row_type', '')) = 'invoice'
  ...
  -- IMP-001: execute — lock existing re-import invoices before paid-floor check and mutations
  FOR v_inv IN
    SELECT DISTINCT COALESCE(inv_row->>'invoice_number', '')
    FROM jsonb_array_elements(p_rows) AS inv_row
    WHERE LOWER(COALESCE(inv_row->>'row_type', '')) = 'invoice'
    ORDER BY 1
  LOOP
    v_existing_invoice_id := NULL;
    v_currency := NULL;

    SELECT i.id, i.currency::text
    INTO v_existing_invoice_id, v_currency
    FROM public.invoices i
    WHERE i.workspace_id = p_workspace_id
      AND i.invoice_number = v_inv
...
COMMENT ON FUNCTION public.internal_import_invoices_grouped(uuid, jsonb, boolean) IS
  'Internal grouped invoice import: positive line-item validation, execute re-import locks public.invoices FOR UPDATE before paid-total guard, existing-client resolution only (no client auto-create), full-batch rollback on execute errors.';
