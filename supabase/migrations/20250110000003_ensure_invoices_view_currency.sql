-- ============================================================================
-- Ensure invoices_view includes currency column
-- ============================================================================
-- 
-- This migration uses CREATE OR REPLACE VIEW to ensure currency is included
-- from the base invoices table. This makes the view resilient to older DBs
-- and ensures currency is always available.
--
-- Currency is selected from invoices.currency with fallback to 'USD' if null.
-- ============================================================================
...
SELECT
  ...
  END AS risk_level,
  client_name,
  currency  -- Currency from invoices table (with USD fallback)
FROM invoice_status;

-- Add comment to document the view
COMMENT ON VIEW public.invoices_view IS 
  'Comprehensive view for invoice status, overdue, and risk calculations. Includes currency from invoices table (fallback to USD). Draft invoices are NEVER treated as overdue or risk, even if due_date is in the past. Display status: void (highest priority), draft (protected from overdue), paid (outstanding <= 0), overdue (sent + outstanding > 0 + due_date < current_date), partially_paid (sent + outstanding > 0 + due_date >= current_date + paid_amount > 0), sent (sent + outstanding > 0 + due_date >= current_date + paid_amount = 0). Risk levels: high (overdue_days >= 60 OR outstanding >= 5000), medium (overdue_days 15-59), low (overdue_days 1-14), NULL (not overdue). Paid amount is sum of payments with status completed/paid/null.';
