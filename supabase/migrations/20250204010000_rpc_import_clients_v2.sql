-- ============================================================================
-- Fix RPC Import Clients: Return per-row errors (no exceptions)
-- ============================================================================
-- 
-- This migration replaces rpc_import_clients to:
-- - NEVER raise exceptions for per-row errors (only for fatal conditions)
-- - Return JSONB array with results for EVERY input row
-- - Use ON CONFLICT with unique indexes for upsert logic
-- - Handle organization_id requirement
-- - Use whatsapp_phone (not phone)
-- 
...
-- 
-- Sample call:
-- SELECT public.rpc_import_clients(
--   'workspace-uuid-here'::uuid,
--   '[{"rowId":"1","name":"Test Client","email":"test@example.com","whatsapp_phone":"+1234567890","company_name":"Test Co","status":"active"}]'::jsonb
-- );
-- Expected output: [{"rowId":"1","status":"ok","client_id":"uuid","error":null}]
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_company_name := NULLIF(TRIM(COALESCE(r->>'company_name', r->>'company')), '');
      ...
      IF v_err IS NULL THEN
        ...
        IF v_email IS NOT NULL THEN
          ...
          -- Ensure archived_at is NULL so it participates in the partial unique index
