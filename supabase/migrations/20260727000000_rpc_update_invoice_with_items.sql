-- ============================================================================
-- Atomic invoice edit: update header + replace line items in one transaction
-- ============================================================================
-- Replaces the non-atomic PostgREST sequence (UPDATE → DELETE → INSERT) used by
-- updateInvoice. Any failure during header update, item delete, or item insert
...
COMMENT ON FUNCTION public.rpc_update_invoice_with_items(
  ...
  'Atomically updates one invoice header and replaces all line items. Requires auth.uid() workspace membership. Rejects archived and fully-paid invoices (outstanding <= 0.01). Money fields are supplied by the application server (calculateInvoiceMoney).';
