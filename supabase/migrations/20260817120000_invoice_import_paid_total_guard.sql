-- ============================================================================
-- Invoice re-import paid-total guard (Gate 2 financial invariant)
-- ============================================================================
-- Rejects updating an existing invoice to a total below effective paid amount.
-- Uses invoices_view paid semantics: completed, paid, NULL; excludes archived;
-- sums COALESCE(net_amount, amount). Tolerance 0.01 (PAYMENT_OVERPAY_TOLERANCE).
-- Checked on dry_run and execute before any invoice/payment mutations.
-- ============================================================================
...
COMMENT ON FUNCTION public.internal_import_invoices_grouped(uuid, jsonb, boolean) IS
  'Internal grouped invoice import: explicit UPDATE/INSERT headers, one DELETE per invoice, all items inserted, paid-total guard on re-import, full-batch rollback on execute errors.';
