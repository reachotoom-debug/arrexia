-- ============================================================================
-- Fix invoices_view to ensure currency column exists and reload schema cache
-- Safe to run multiple times (idempotent)
-- ============================================================================

-- Step 1: Verify current column list
DO $$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE 'invoices_view.currency exists: %', has_currency;
END $$;

-- Step 2: Recreate view with currency column
...
SELECT
  ...
  END AS risk_level,
  client_name,
  currency  -- Currency from invoices table (i.currency AS currency)
FROM invoice_status;

-- Step 3: Reload PostgREST schema cache
SELECT pg_notify('pgrst', 'reload schema');

-- Step 4: Sanity check - verify currency column exists and view works
SELECT 
  column_name,
  data_type,
  ordinal_position
...
COMMENT ON VIEW public.invoices_view IS 
  'Comprehensive view for invoice status, overdue, and risk calculations. Includes currency from invoices table (i.currency AS currency). Draft invoices are NEVER treated as overdue or risk, even if due_date is in the past. Display status: void (highest priority), draft (protected from overdue), paid (outstanding <= 0), overdue (sent + outstanding > 0 + due_date < current_date), partially_paid (sent + outstanding > 0 + due_date >= current_date + paid_amount > 0), sent (sent + outstanding > 0 + due_date >= current_date + paid_amount = 0). Risk levels: high (overdue_days >= 60 OR outstanding >= 5000), medium (overdue_days 15-59), low (overdue_days 1-14), NULL (not overdue). Paid amount is sum of payments with status completed/paid/null.';
