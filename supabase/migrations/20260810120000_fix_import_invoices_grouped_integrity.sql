-- ============================================================================
-- P0: Invoice import financial integrity + batch atomicity + net-new entitlement
-- ============================================================================
-- Fixes:
-- 1. Multi-item invoices: DELETE invoice_items once per invoice, then INSERT ALL items
-- 2. Execute path: any runtime error RAISEs (full batch rollback)
-- 3. Entitlement preflight counts NET-NEW invoices only (updates excluded)
-- 4. Explicit UPDATE/INSERT invoice headers (avoids BEFORE INSERT trigger on upsert updates)
-- ============================================================================
...
COMMENT ON FUNCTION public.import_invoices_grouped(uuid, jsonb, boolean) IS
  'Canonical invoice import RPC: net-new entitlement preflight on execute, delegates to internal_import_invoices_grouped. Execute failures roll back the entire batch.';
COMMENT ON FUNCTION public.internal_import_invoices_grouped(uuid, jsonb, boolean) IS
  'Internal grouped invoice import: explicit UPDATE/INSERT headers, one DELETE per invoice, all items inserted, full-batch rollback on execute errors.';
