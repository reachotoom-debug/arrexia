...
function calculateDso(
  ...
  let interpretation =
    "Lower DSO generally means faster collections. Higher DSO may indicate slower collections and cash flow risk.";

  if (dso <= 30) {
    interpretation =
      "Lower DSO means faster collections. Strong invoice tracking and timely payment reminders often help keep DSO low.";
  } else if (dso <= 45) {
    interpretation =
    ...
    interpretation =
      "Higher DSO may indicate slower collections and cash flow risk. Consider tightening follow-up workflows and reminder sequences.";
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 sm:p-8">
        <p className="text-sm font-medium uppercase tracking-wide text-slate-500">Your DSO</p>
        {formattedResult ? (
          <>
            <p className="mt-4 text-5xl font-semibold tracking-tight text-slate-900">
              {formattedResult}
              <span className="ml-2 text-2xl font-medium text-slate-500">days</span>
            </p>
            <p className="mt-5 text-sm leading-relaxed text-slate-600">{result?.interpretation}</p>
          </>
        ) : (
