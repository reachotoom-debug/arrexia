...
export function AuthSocialLogin({
  ...
  const [oauthError, setOauthError] = useState<string | null>(() =>
    getOAuthCallbackErrorMessage(initialError)
  );
  const [loadingProvider, setLoadingProvider] = useState<OAuthProvider | null>(null);

  useEffect(() => {
    if (initialError) {
      stripCredentialsFromAuthUrl({ stripOAuthErrors: true });
    }
  }, [initialError]);
  ...
  return (
    <div className={authSocialSectionClass}>
      <button
        type="button"
      ...
      {oauthError ? (
        <div className={authOAuthErrorClass} role="alert">
          {oauthError}
        </div>
      ) : null}
